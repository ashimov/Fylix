...
async def test_audit_csv_content_type() -> None:
    ...
    async with httpx.AsyncClient(base_url=BASE) as c:
        await _login(c, email, secret)
        r = await c.get(
            "/api/admin/audit.csv?since=2020-01-01T00:00:00%2B00:00&until=2020-03-31T00:00:00%2B00:00"
        )
    assert r.status_code == 200
...
async def test_audit_csv_export_records_admin_action() -> None:
    ...
    async with SessionLocal() as s:
        ...
        actions = (
            (await s.execute(select(AdminAction).where(AdminAction.action == "export_audit_csv")))
            .scalars()
            .all()
        )
...
async def test_admin_actions_list_returns_own_actions() -> None:
    ...
    async with httpx.AsyncClient(base_url=BASE) as c:
        ...
        await c.patch(
            "/api/admin/settings", json={"max_recipients": 10}, headers={"X-CSRF-Token": csrf}
        )
        r = await c.get("/api/admin/admin-actions")
    assert r.status_code == 200
    body = r.json()
