...
async def test_missing_staging_file_flips_transfer_to_infected() -> None:
    ...
    async with httpx.AsyncClient(base_url=BASE, verify=False) as c:
        ...
        patch = await c.patch(file_url, content=b"A" * 50,
    ...
    async with SessionLocal() as session:
        tf = (await session.execute(
            select(TransferFile).where(TransferFile.transfer_id == tid)
        )).scalar_one()
    ...
    async with SessionLocal() as session:
        ...
        t.created_at = datetime.now(timezone.utc) - timedelta(seconds=30)
        await session.commit()
    ...
    async with SessionLocal() as session:
        flipped = await run_defender_poll_once(
            session=session, staging=staging, alerts=alerts, min_age_seconds=10,
        )
    assert flipped == 1
    ...
    async with SessionLocal() as session:
        ...
        audits = (await session.execute(
            select(AuditLog).where(AuditLog.transfer_id == tid)
        )).scalars().all()
